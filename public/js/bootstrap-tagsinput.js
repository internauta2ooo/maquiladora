	...
	(function ($, window, document, undefined) {
		...
		AmsifySuggestags.prototype = {
			...
			printValues: function () {
				...
				console.log(document.getElementsByName('coordinadoselect'));
				console.log($('[name ="coordinadoselect"]'));
				$('[name ="coordinadoselect"]').empty();
				$('#cordinadostabla').empty();
				var coordinados = this.tagNames;
				coordinados.forEach(
					function (key) {
						console.log(key);
						// $('#cordinadostabla').append('<option value="1">' + key + '</option>');
						$('[name ="coordinadoselect"]').append('<option value="1">' + key + '</option>');
					}
				);
